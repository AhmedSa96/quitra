...
- **Models:** `freezed` + `json_serializable` (Immutable types &
  boilerplate-free models)
- **Networking:** `supabase_flutter`
...
- **DataSources:** Raw API or Local DB calls (Supabase/Isar/Hive).
- **Repository Implementations:** Logic to fetch from data source and convert
  Models back to Entities.
...
- **States:** Always use Freezed for Union states (Initial, Loading, Success,
  Failure).
...
Catch `Exceptions` in the Data layer and return `Failures` (Either type or
Result type) to the Domain/Presentation layer.

### Model Generation

Always run: `dart run build_runner build --delete-conflicting-outputs` whenever
modifying Freezed/Injectable files.
